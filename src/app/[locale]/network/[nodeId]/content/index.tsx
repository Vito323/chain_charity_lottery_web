...
const NodeDetail: React.FC<NodeDetailProps> = ({ nodeId }) => {
  ...
  const isGenesis =
    nodeIdLower === "genesis" || nodeIdLower.includes("genesis");
  const isStandard =
    nodeIdLower === "standard" ||
    nodeIdLower.includes("standard") ||
    nodeIdLower.includes("normal");
  ...
  const nodeInfo = {
    name: isGenesis
      ? tNetwork("nodeTiers.genesis.name")
      : isStandard
      ? tNetwork("nodeTiers.standard.name")
      : isSuper
      ? tNetwork("nodeTiers.super.name")
      : "Node",
    title: isGenesis
    ...
    <div className="min-h-screen bg-linear-to-b from-slate-950 via-slate-900 to-slate-950">
                  ...
                  >
                    ...
                    <div className="relative h-2 rounded-full bg-white/10 overflow-hidden">
                      <motion.div
                        ...
                        className="absolute inset-y-0 left-0 bg-linear-to-r from-purple-500 via-pink-500 to-purple-500 rounded-full"
                      />
                    </div>
                    {nodeStats.remaining > 0 && (
                      <p className="text-xs text-white/60 text-center mt-1.5">
                        {nodeStats.remaining} {t("stats.remainingLowercase")}
                      </p>
                    )}
                  </motion.div>
                )}
                  ...
                  <motion.div
                    ...
                    className={`relative rounded-xl sm:rounded-2xl border-2 ${
                      selectedCertificate === cert.id
                        ...
                        : cert.borderColor
                    } bg-linear-to-br ${cert.gradient} p-3 sm:p-4 md:p-6 aspect-[3/4] flex flex-col justify-between shadow-xl hover:scale-105 transition-all duration-300 cursor-pointer`}
                  >
                    {selectedCertificate === cert.id && (
                    ...
                    <div className="relative z-10">
                      ...
                      {cert.subtitle && (
                        <p className="text-[10px] xs:text-xs text-white/70 break-words">
                          {cert.subtitle}
                        </p>
                      )}
                    </div>
      ...
      <NodePurchaseModal
        ...
        onPurchaseSuccess={(nodeType, certificateId) => {
          ...
          setPurchaseData({
            nodeType,
            certificateId: certificateId || selectedCertificate,
